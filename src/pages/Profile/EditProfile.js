...
export default function EditProfile() {
    ...
    function getUser(){
        axios.get(`http://localhost:80/book_app_redux-main/back_end/users.php/${id}`)
        .then(response => {
      ...
      const handleEditUserSubmit  = async (e) => {
        ...
        try {
          const response = await axios.post(
            `http://localhost:80/book_app_redux-main/back_end/editUserProfile.php/${id}`, formEditData
          );
          console.log(response.data);
          navigate(`/account`);
